...
// Kopiert und bearbeitet aus dem Download unter http://www.labbookpages.co.uk/audio/javaWavFiles.html
...
public class WavFile
{
	...
	private long numSamples;					// Number of frames within the data section
	...
	private int sampleRate;				// 4 bytes unsigned, 0x00000001 (1) to 0xFFFFFFFF (4,294,967,295)
	...
	private long sampleCounter;				// Current number of frames read or written
	private int seconds;
	...
	public long getSamplesRemaining()
	{
		return numSamples - sampleCounter;
	}
	...
	public static WavFile getWavFile(String file_path) {
    	WavFile wavFile;
		try {
	...
	public static WavFile newWavFile(File file, int numChannels, long numFrames, int validBits, int sampleRate) throws IOException, WavFileException
	{
		// Instantiate new Wavfile and initialise
		WavFile wavFile = new WavFile();
		wavFile.file = file;
		wavFile.numChannels = numChannels;
		wavFile.numSamples = numFrames;
	...
	public static WavFile openWavFile(File file) throws IOException, WavFileException
	{
		...
		while (true)
		{
			...
			else if (chunkID == DATA_CHUNK_ID)
			{
				...
				wavFile.numSamples = chunkSize / wavFile.blockAlign;
		...
		wavFile.seconds = (int) (wavFile.getNumSamples() / wavFile.getSampleRate());

		return wavFile;
	}
	
	public double[] getWavAmplitude() {
		double[] buffer = new double[0];
		
    	try {
			buffer = new double[(int) getNumSamples()];
			int framesRead;
			do {
				framesRead = readFrames(buffer, buffer.length);
			} while (framesRead != 0);
			close();
		}
		catch (Exception e) {
	...
	/*
	 * Gibt die Dauer der Wav-Datei in Sekunden zurück
	 */
	...
	public int readFrames(int[] sampleBuffer, int offset, int numFramesToRead) throws IOException, WavFileException
	{
		...
		{
			if (sampleCounter == numSamples) return f;
